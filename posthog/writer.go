...
func NewWriterConfigFromEnv(format *golog.Format, filter golog.LevelFilter, valsAsMsg bool, extra map[string]any) (*WriterConfig, error) {
	...
	return NewWriterConfig(client, format, filter, valsAsMsg, extra), nil
}

// NewWriterConfig returns a new WriterConfig for PostHog.
// Any values passed as extra will be added to every log message.
func NewWriterConfig(client posthog.Client, format *golog.Format, filter golog.LevelFilter, valsAsMsg bool, extra map[string]any) *WriterConfig {
	return &WriterConfig{
		format:    format,
		filter:    filter,
		client:    client,
...
func (w *Writer) CommitMessage() {
	...
	if w.message.Len() > 0 {
		...
		w.config.client.Enqueue(posthog.Capture{
			DistinctId: "system", // TODO: Make configurable or extract from context
			Event:      "log_message",
