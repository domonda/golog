...
// func (f *FilteredWriter) BeginMessage(logger *Logger, t time.Time, level Level, prefix, text string) Writer {
// 	if !f.filter.IsActive(level) {
// 		return NopWriter
// 	}
// 	return NewFilteredWriter(f.filter, f.wrapped.BeginMessage(logger, t, level, prefix, text))
// }

// func (f *FilteredWriter) CommitMessage() {
// 	f.wrapped.CommitMessage()
// }
